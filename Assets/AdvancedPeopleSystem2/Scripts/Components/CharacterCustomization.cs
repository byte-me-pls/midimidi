...
namespace AdvancedPeopleSystem
{
    ...
    public class CharacterCustomization : MonoBehaviour
    {
        ...
        public Action<CharacterElementType, int> OnSelectNewElement;
        public Action<CharacterElementType> OnClearElement;
        ...
        public void SetElementByIndex(CharacterElementType type, int index)
        {
            ...
            if(type == CharacterElementType.Head)
            {
                SetHeadByIndex(index);
                RecalculateShapes();
                return;
            }
            ...
            OnSelectNewElement?.Invoke(type, index);
        }
        ...
        {
            SetBodyElementByIndex("Hair", index);
        }
        private void SetHeadByIndex(int index)
        {
            SetBodyElementByIndex("Head", index);
        }
        /// <summary>
        ...
        {
            SetBodyElementByIndex("Beard", index);           
        }

        private void SetBodyElementByIndex(string partName, int index) 
        {
            CharacterPart characterPart = GetCharacterPart(partName);

            CharacterElementType type = (CharacterElementType)(Enum.Parse(typeof(CharacterElementType), partName, true));

            if (characterPart == null || characterPart.skinnedMesh.Count <= 0)
                return;

            var presets = GetElementsPresets(type);
            if (index != -1 && presets != null)
            {
                var preset = presets.ElementAtOrDefault(index);

                if (preset == null)
                {
                    Debug.LogError($"{partName} with index {index} not found");
                    return;
                }

                var verified = VerifyPreset(preset);
                ...
                for (var i = 0; i < (MaxLODLevels - MinLODLevels) + 1; i++)
                {
                    int lod_index = i + MinLODLevels;

                    if (!characterPart.skinnedMesh[i].gameObject.activeSelf)
                    {
                        characterPart.skinnedMesh[i].gameObject.SetActive(true);
                    }
                    characterPart.skinnedMesh[i].sharedMesh = preset.mesh[lod_index];

                    if (preset.mats != null && preset.mats.Length > 0)
                    {
                        var mats = preset.mats.ToList();

                        characterPart.skinnedMesh[i].sharedMaterials = mats.ToArray();

                        for (var m = 0; m < mats.Count; m++)
                            if (mats[m].name == _settings.bodyMaterial.name)
                                characterPart.skinnedMesh[i].SetPropertyBlock(bodyPropertyBlock, m);
                    }
                }
            }
            else
            {
                foreach (var sMesh in characterPart.skinnedMesh)
                {
                    sMesh.sharedMesh = null;
                    sMesh.gameObject.SetActive(false);
                }
            }
            characterSelectedElements.SetSelectedIndex(type, index);
        }
        ...
        public CharacterCustomizationSetup GetSetup()
        {
            ...
            ccs.selectedElements.Head = characterSelectedElements.Head;
        ...
        {
            return _settings.GetCharacterElementsPresets(type);
        }
        /// <summary>
        /// Play animation
        /// </summary>
